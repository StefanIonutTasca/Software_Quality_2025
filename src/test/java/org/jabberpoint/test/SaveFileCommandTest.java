...
class SaveFileCommandTest {
  ...
  void shouldExecuteFileChooserWhenInGUIEnvironment() throws Exception {
    ...
    Assumptions.assumeFalse(
        GraphicsEnvironment.isHeadless(), "Skipping GUI test in headless environment");

    // Create a temp file
    File tempFile = tempDir.resolve("save.xml").toFile();

    // Mock JFileChooser
    try (MockedConstruction<JFileChooser> mockConstruction =
            mockConstruction(
                JFileChooser.class,
                (mock, context) -> {
                  when(mock.showSaveDialog(any(Frame.class)))
                      .thenReturn(JFileChooser.APPROVE_OPTION);
                  when(mock.getSelectedFile()).thenReturn(tempFile);
                });
        MockedConstruction<XMLPresentationLoader> mockedLoader =
            mockConstruction(
                XMLPresentationLoader.class,
                (mock, context) -> {
      ...
      // Assert
      assertEquals(
          1, mockedLoader.constructed().size(), "XMLPresentationLoader should be constructed");
  ...
  void shouldHandleWhenUserCancelsFileChooser() throws Exception {
    ...
    try (MockedConstruction<JFileChooser> mockConstruction =
        ...
        MockedConstruction<XMLPresentationLoader> mockedLoader =
            mockConstruction(XMLPresentationLoader.class)) {
      ...
      // Assert
      assertTrue(
          mockedLoader.constructed().isEmpty(), "XMLPresentationLoader should not be constructed");
    }
  }
            ...
            mockConstruction(
                ...
                (mock, context) -> {
                  doThrow(new IOException("Test exception"))
                      .when(mock)
                      .savePresentation(any(Presentation.class), anyString());
                });
        MockedStatic<JOptionPane> mockedOptionPane = mockStatic(JOptionPane.class)) {

      // Mock JOptionPane to avoid showing dialog
      mockedOptionPane
          .when(() -> JOptionPane.showMessageDialog(any(), anyString(), anyString(), anyInt()))
