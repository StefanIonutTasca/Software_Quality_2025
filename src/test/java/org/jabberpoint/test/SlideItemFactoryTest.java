...
class SlideItemFactoryTest {
  ...
  void createSlideItemShouldCreateBitmapItem() {
    ...
    assertEquals(
        "test.jpg", ((BitmapItem) item).getImageName(), "Image name should be set correctly");
  }
  ...
  void createSlideItemShouldBeCaseInsensitiveForImage(String imageType) {
    ...
    // Assert
    assertTrue(
        item instanceof BitmapItem, "Created item should be a BitmapItem regardless of case");
  }
  ...
  void createSlideItemShouldThrowExceptionForUnknownType() {
    ...
    assertTrue(
        exception.getMessage().contains("Unknown slide item type"),
