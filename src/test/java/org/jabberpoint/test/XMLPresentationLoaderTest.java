...
class XMLPresentationLoaderTest {
  ...
  void loadPresentationShouldParseXmlAndPopulatePresentation() throws IOException {
    ...
    assertEquals(
        "test_image.png",
        ((BitmapItem) item2).getName(),
        "Second item image name should be correct");
  ...
  void loadPresentationShouldHandleInvalidLevelAttributeGracefully() throws IOException {
    ...
    assertTrue(
        errContent.toString().contains("Number format exception for level"),
  ...
  void loadPresentationShouldHandleUnknownItemKindGracefully() throws IOException {
    ...
    // Verify error message was logged
    assertTrue(
        errContent.toString().contains("Unknown element type"),
  ...
  void loadPresentationShouldThrowIOExceptionForFileNotFound() {
    ...
    assertTrue(
        exception.getMessage().contains("non_existent_file.xml")
  ...
  void savePresentationShouldWritePresentationToXmlFile() throws IOException {
    ...
    assertTrue(
        savedContent.contains("<?xml version=\"1.0\"?>"), "XML declaration should be present");
    assertTrue(savedContent.contains("<presentation>"), "Root element should be present");
    assertTrue(
        savedContent.contains("<showtitle>Saved Presentation</showtitle>"),
        "Show title should be present");
    assertTrue(savedContent.contains("<slide>"), "Slide element should be present");
    assertTrue(
        savedContent.contains("<title>Saved Slide 1</title>"), "Slide 1 title should be present");
    assertTrue(
        savedContent.contains("<item kind=\"text\" level=\"1\">Saved Text Item 1</item>"),
        "Text item should be present with correct attributes");
    assertTrue(
        savedContent.contains("<item kind=\"image\" level=\"2\">saved_image.png</item>"),
        "Image item should be present with correct attributes");
    assertTrue(
        savedContent.contains("<title>Saved Slide 2</title>"), "Slide 2 title should be present");

    // Check that DOCTYPE declaration is not present (memory mentioned removing it)
    assertFalse(
        savedContent.contains("<!DOCTYPE"),
  ...
  void savePresentationShouldHandleUnsupportedSlideItemTypes() throws IOException {
    ...
    final SlideItem mockItem =
        new SlideItem() {
          ...
          public Rectangle getBoundingBox(
              ...
              float scale,
              org.jabberpoint.src.model.Style myStyle) {
    ...
    assertTrue(savedContent.contains("<presentation>"), "Root element should be present");
    assertTrue(
        savedContent.contains("<showtitle>Mock Presentation</showtitle>"),
        "Show title should be present");
    assertTrue(savedContent.contains("<slide>"), "Slide element should be present");
    ...
    assertFalse(
        savedContent.contains("<item kind=\"text\" level=\"1\">MockSlideItem</item>"),
