...
class XMLAccessorTest {
  ...
  void loadFileShouldParseXmlAndPopulatePresentation() throws IOException {
    ...
    assertEquals(
        "test_image.png", ((BitmapItem) item2).getName(), "Second item image name should be set");
  ...
  void loadFileShouldHandleInvalidLevelAttributeGracefully() throws IOException {
    ...
    try {
      ...
      assertTrue(
          errContent.toString().contains("Number Format Exception"),
          "Should show NumberFormatException message");

      // Check presentation content (should still work with default level)
      assertEquals(
          "Invalid Level Test", presentation.getTitle(), "Presentation title should be set");
  ...
  void loadFileShouldHandleUnknownItemKindGracefully() throws IOException {
    ...
    try {
      ...
      assertEquals(
          "Unknown Kind Test", presentation.getTitle(), "Presentation title should be set");
      assertEquals(1, presentation.getSize(), "Presentation should have 1 slide");
  ...
  void saveFileShouldWritePresentationToXmlFile() throws IOException {
    ...
    assertTrue(
        fileContent.contains("<showtitle>Saved Presentation</showtitle>"),
    ...
    assertTrue(
        fileContent.contains("<item kind=\"text\" level=\"1\">Test Text Item</item>"),
        "XML should contain text item");
    ...
    assertEquals(
        "Test Text Item",
        ((TextItem) loadedItem).getText(),
        "Loaded text item content should match");
  ...
  void saveFileShouldHandleUnsupportedSlideItemTypes() throws IOException {
    ...
    try {
      ...
      assertTrue(
          errorOutput.contains("unsupported") || errorOutput.contains("Unknown"),
