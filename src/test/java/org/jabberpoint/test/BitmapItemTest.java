...
class BitmapItemTest {
  ...
  @Mock private ImageObserver mockObserver;
  ...
  void setUp() throws IOException {
    ...
    when(mockObserver.imageUpdate(any(), anyInt(), anyInt(), anyInt(), anyInt(), anyInt()))
        .thenReturn(true);
  }
  ...
  void constructorShouldInitializeWithLevelAndImageName() {
    ...
    assertEquals(
        tempDir.resolve(testImageName).toString(),
        bitmapItem.getName(),
        "Image name should be initialized correctly");
  }
  ...
  void getImageNameShouldReturnImageName() {
    ...
    assertEquals(
        bitmapItem.getName(),
        bitmapItem.getImageName(),
        "getName and getImageName should return the same value");
  }
  ...
  void drawShouldRenderImageWithCorrectDimensions() throws Exception {
    ...
    // Verify the image was drawn (we can't verify exact parameters without accessing private
    // fields)
    verify(mockGraphics)
        .drawImage(eq(testImage), anyInt(), anyInt(), anyInt(), anyInt(), eq(mockObserver));
  }
  ...
  void toStringShouldReturnFormattedStringRepresentation() {
    ...
    assertEquals(
        expected, bitmapItem.toString(), "toString should return formatted representation");
  }
  ...
  void shouldHandleNullImageNameGracefully() {
    ...
    assertDoesNotThrow(
        () -> nullNameItem.draw(10, 10, 1.0f, mockGraphics, realStyle, mockObserver));
  ...
  void shouldReturnImageNameCorrectly() {
    ...
    assertTrue(
        toStringResult.contains(String.valueOf(testLevel)), "toString should include the level");
  }
}
