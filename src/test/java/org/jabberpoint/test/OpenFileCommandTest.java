...
class OpenFileCommandTest {
  ...
  @Mock private PresentationLoader mockLoader;
  ...
  void shouldExecuteFileChooserWhenInGUIEnvironment() throws Exception {
    ...
    try (MockedConstruction<JFileChooser> mockConstruction =
      ...
      try (MockedStatic<PresentationLoaderFactory> mockedFactory =
          mockStatic(PresentationLoaderFactory.class)) {
        mockedFactory
            .when(() -> PresentationLoaderFactory.createLoader("xml"))
    ...
    try (MockedConstruction<JFileChooser> mockConstruction =
      ...
      try (MockedStatic<PresentationLoaderFactory> mockedFactory =
        ...
        doThrow(new IOException("Test exception"))
            .when(mockLoader)
            .loadPresentation(any(Presentation.class), anyString());
        mockedFactory
            .when(() -> PresentationLoaderFactory.createLoader("xml"))
            .thenReturn(mockLoader);
