...
class SlideTest {
  ...
  @Mock private SlideItem mockSlideItem;

  @Mock private Rectangle mockRectangle;
  ...
  void setUp() {
    ...
    when(mockSlideItem.getBoundingBox(
            any(Graphics.class), any(ImageObserver.class), anyFloat(), any(Style.class)))
  ...
  void drawShouldRenderSlideTitleAndAllItems() {
    ...
    verify(mockSlideItem, atLeastOnce())
        .draw(
            anyInt(), anyInt(), anyFloat(), eq(mockGraphics2D), any(Style.class), eq(mockObserver));
  }
